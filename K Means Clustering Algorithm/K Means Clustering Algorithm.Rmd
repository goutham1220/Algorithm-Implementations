...
We first initialize variables for the number of points, number of inherent clusters in the data, number of desired clusters, number of iterations OF KMEANS, and the spread of the data. Initializing these variables allows for the most customization of the algorithm when testing it with data. 
...
kmeans_simulation = function(npoints, nclusters, niterations, nclusters.data = 4, spread = 5, meanValue = 50){
  ...
  for(i in 1:nclusters.data){ # run following code for every inherent cluster
   ## generates random X and Y values
    xvalues = c(xvalues, rnorm(npoints/nclusters.data, mean = (25*i), sd = spread)) #clustered x values
    yvalues = c(yvalues, rnorm(npoints/nclusters.data, mean = (100*i)/nclusters.data, sd = spread)) # clustered y values
  ...
  xy =  dataset[sample(1:npoints, nclusters),] # initializes centroids to random pts from dataset
  color = 1:nrow(xy) 
  ...
  for(j in 1:niterations){ # runs for specified number of iterations
    ...
    alldistances = apply(centroids, MARGIN = 1, distance, matrix = dataset) #uses apply() to calculate distances between centroids & datapts
    clusterAssignment = apply(alldistances, MARGIN = 1, which.min) # assigns datapt to cluster based on min distance
    cost[j] = sum(sapply(1:nclusters, # uses sapply() and sum() to track cost using total distances
                         function(x) sum(distance(dataset[clusterAssignment == x,], centroids[x,]))))
    centroids[,1:2] = sapply(1:nclusters, # uses sapply() to reassign centroid coordinates to mean of dataset coordinates
                             function(x) mean(dataset[clusterAssignment == x,1:2]))

  }
  
  return (list(cost, clusterAssignment, centroids, dataset, centroids_init)) # returns list of various values
}
```
...
```{r}
list = kmeans_simulation(numPoints, numClusters, numIterations, numClusters.data, spread, meanValue)
```
...
As we can see, the graph appears to plateau at 5 clusters, which is the optimal number of clusters for this simulation.
